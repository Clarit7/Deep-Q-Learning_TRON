...
from tron.AC_map import Map, Tile
...
class PositionPlayer:
    ...
    def crashed(self):
        ...
        if self.id == 1:
            return Tile.PLAYER_ONE_CRASHED
        elif self.id == 2:
            return Tile.PLAYER_TWO_CRASHED
    ...
    def next_frame(self,action_p1,action_p2):
        ...
        for (id, pp) in enumerate(self.pps):
            ...
            if pp.position[0] < 0 or pp.position[1] < 0 or \
                ...
                pp.alive = False
                map_clone[pp.position[0], pp.position[1]] = pp.crashed()
        ...
        if pos_1 == pos_2:
            map_clone[pos_1[0], pos_2[1]] = Tile.HEAD_CRASHED
            for (id, pp) in enumerate(self.pps):
                pp.alive = False
