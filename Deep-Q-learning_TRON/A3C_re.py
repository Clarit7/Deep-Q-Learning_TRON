...
class Net(nn.Module):
    def __init__(self):
        ...
        self.actor1 = nn.Linear(64, 32)
        self.actor2 = nn.Linear(32, 4)

        self.critic1 = nn.Linear(64, 32)
        self.critic2 = nn.Linear(32, 1)
    ...
    def forward(self, x):
        ...
        h1 = self.relu(self.actor1(x))
        h1 = self.dropout(h1)
        actor_output = self.actor2(h1)

        h2 = self.relu(self.critic1(x))
        h2 = self.dropout(h2)
...
class ActorPlayer(Player):
    def __init__(self):
        super(ActorPlayer, self).__init__()
...
def make_game():
    ...
    player2 = ActorPlayer()
    #
    game = Game(MAP_WIDTH, MAP_HEIGHT, [PositionPlayer(1, player1, [x1, y1]),
...
class Environment:
    def run(self):
        ...
        rollouts = RolloutStorage(NUM_ADVANCED_STEP, NUM_PROCESSES * 2)  # rollouts 객체
        episode_rewards = torch.zeros([NUM_PROCESSES * 2, 1])  # 현재 에피소드의 보상
        final_rewards = torch.zeros([NUM_PROCESSES * 2, 1])  # 마지막 에피소드의 보상
        obs_np = np.zeros([NUM_PROCESSES * 2, 12, 12])  # Numpy 배열
        reward_np = np.zeros([NUM_PROCESSES * 2, 1])  # Numpy 배열
        done_np = np.zeros([NUM_PROCESSES * 2, 1])  # Numpy 배열
        each_step = np.zeros(NUM_PROCESSES * 2)  # 각 환경의 단계 수를 기록
